...
const CommintsSection = ({ productId, comments=[], currentUserId }) => {
  ...
  const handleSubmit = (e) => {
    ...
    createComment.mutate({ productId, content },{
      onSuccess: () => {
        setContent("");
      }
    });
            ...
            comments.map((comment)=> (
                <div className="chat chat-start " key={comment.id}>
                  ...
                  <div className="chat-header text-xs opacity-70 mb-2">
                    {comment.user?.name}
                    <time className="ml-2 text-x5 opacity-50">
...
export default CommintsSection